...
export default async function RevisaoSemanalPage() {
  ...
  return <div className="flex flex-col gap-5 p-4"><div><p className="text-label-md uppercase text-muted-foreground">Revisão Semanal</p><h1 className="text-headline-md font-bold">Planejado versus realizado</h1><p className="text-body-sm text-muted-foreground">Medição isolada ou troca de método não conta como resposta corporal.</p></div>
    ...
    <form action={gerarRevisaoSemanal} className="grid gap-3"><label className="text-body-sm">Como foi sua recuperação?<select name="recuperacao" defaultValue="3" className="mt-1 w-full rounded-md border bg-background p-3"><option value="1">Muito baixa</option><option value="2">Baixa</option><option value="3">Regular</option><option value="4">Boa</option><option value="5">Muito boa</option></select></label><label className="text-body-sm">As recomendações foram úteis?<select name="utilidade" defaultValue="3" className="mt-1 w-full rounded-md border bg-background p-3"><option value="1">Nada úteis</option><option value="2">Pouco úteis</option><option value="3">Parcialmente úteis</option><option value="4">Úteis</option><option value="5">Muito úteis</option></select></label><Button className="w-full">{revisao ? "Atualizar revisão" : "Iniciar revisão"}</Button></form><Button asChild variant="ghost"><Link href="/progresso">Voltar ao Progresso</Link></Button></div>;
}
