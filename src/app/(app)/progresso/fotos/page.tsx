...
export default async function FotosProgressoPage() {
  ...
  const fotos = await Promise.all(panorama.fotos.map(async (foto) => ({ id: foto.id, pose: foto.pose, data: foto.observadoEm.toLocaleDateString("pt-BR"), condicoes: foto.condicoes, protocoloVersao: foto.protocoloVersao, url: await storage.urlLeitura(foto.objectKey, 600) })));
