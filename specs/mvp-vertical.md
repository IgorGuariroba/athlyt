...
64. Como usuário, quero registrar circunferências com instruções padronizadas do ponto anatômico, postura e tensão da fita, para tornar comparações entre medições mais reproduzíveis sem repetir a leitura na mesma sessão (ADR 0007).
...
- As medições serão agrupadas em um Check-in Diário único decidido pela Cadência Adaptativa: prontidão sempre, peso quando solicitado, cintura no máximo semanalmente e conjunto completo de circunferências/fotos no máximo mensalmente; registro avulso continua possível. Circunferências terão protocolo visual e uma leitura por região, registrada com qualidade `moderada`; leituras adicionais são opcionais e, quando existirem, valem pela mediana (ADR 0007).
...
- O protocolo de circunferências será testado com leitura única, leituras adicionais concordantes, recusa de leituras muito divergentes, mediana, assimetria dentro da tolerância e mudança súbita com sintomas acionando segurança.
