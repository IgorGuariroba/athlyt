...
| Medições | Check-in Diário agrupado via Cadência Adaptativa; cartão no Início + Entrada Planejada no Diário | MacroFactor 016–019 (pesagem) |
...
Login Google → allowlist (negado = mensagem clara, sem persistência) → triagem em cascata (dados físicos, objetivo, experiência, **Importação de Histórico opcional**, disponibilidade, academia/equipamentos, saúde/lesões, alimentação, rotina) → resumo do que falta e do que destrava (Modo Conservador) → **geração do Plano Ativo** → revisão (resumo, dia a dia, substituições, estratégia nutricional) → ativação.

Abandono no meio da cascata: o app entra utilizável em Modo Conservador; o restante vira cartão "completar perfil" no Início.
...
Check-in Diário agrupado: prontidão (1 toque por dimensão) + peso quando solicitado + circunferências (guia padronizado) e fotos (guia de pose/enquadramento + consentimento por operação) nos dias devidos, conforme Cadência Adaptativa. Registro avulso sempre possível pelo +.

### F — Progresso e Revisão Semanal (telas 064–074)

Aba Progresso: cartão Strategy (Plano Ativo, círculo até a Revisão, Experimento em andamento) + gráficos configuráveis (força, tendência de peso por média móvel, circunferências, fotos lado a lado, hexágono de volume semanal) + avaliação visual por critérios separados com faixa probabilística. Revisão Semanal: planejado vs realizado → Scorecard → evidências que sustentam/enfraquecem → proposta (auto-aplicada se pequena; aprovação se estrutural/Experimento) → versionamento + Trilha de Decisão. Rollback ao Plano Estável.
...
- **Modo Conservador**: badge global + cartão explicando capacidades limitadas e o que destrava.
