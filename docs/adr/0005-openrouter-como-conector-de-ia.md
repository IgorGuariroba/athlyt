...
- Duas armadilhas só apareceram na verificação real, ambas silenciosas: (a) o `@ai-sdk/openai-compatible` assume `supportsStructuredOutputs: false` e, sem a flag ligada, descarta o `json_schema` e passa a pedir JSON por instrução — o modelo responde JSON válido, porém com campos inventados; (b) declarar `structured_outputs` no catálogo não garante que o modelo cumpra, e `openai/gpt-oss-20b:free` devolveu prosa em 3 de 3 tentativas. Daí a regra: modelo novo só entra no catálogo depois de passar na verificação real.
